...
const findAndRetrieveNodes = (result, selector, getChildNodes = false) => {
...
const formatDeeplyNested = (selectors, selector) => {
  const children = findAndRetrieveNodes([], selector, true)
  ...
  if (isLast) {
    ...
    const matchingChildren = potentialSiblings
...
const formatMultipleSelectors = (selectors) => {
  ...
  const matchingNodes = formattedSelectors.reduce((r, s) => findAndRetrieveNodes(r, s), [])
